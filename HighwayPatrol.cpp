...
void HighwayPatrol::pullOver( Vehicle* v, bool willArrest, Highway* h )
{
    ...
    if( willArrest )
    {
        ...
        if( auto* car = dynamic_cast<Car*>(v) ) type = "CAR";
        else if( auto* motorcycle = dynamic_cast<Motorcycle*>(v) ) type = "MOTORCYCLE";
        else if( auto* truck = dynamic_cast<SemiTruck*>(v) ) type = "TRUCK";
